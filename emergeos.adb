...
with Types;
with Holographic_Memory;  -- Holographic_Vectors not needed here
with Entities;
package body EmergeOS is
   ...
   procedure Put_Float (F : Float) is
      Whole : constant Integer := Integer(F);  -- FIXED: now constant
      Fraction : Integer := Integer((F - Float(Whole)) * 100.0);
   begin
      Put_Natural (abs Whole);
      Console_Put_Char ('.');
      if Fraction < 0 then
         Fraction := -Fraction;
      end if;
